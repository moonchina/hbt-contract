...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    ...
    context('将ERC/LP令牌添加到字段中', () => {
        ...
        it('为每个投资者发放收益', async () => {
           ...
           //   Alice should have: (4*1000 + 4*1/3*1000 + 2*1/6*1000)*0.9 = 5099
            //   minter should have: (4*1000 + 4*1/3*1000 + 2*1/6*1000)*0.1 = 566
            //   MasterChef should have the remaining: 10000 - 5666 = 4334
            //   MasterChef should have the remaining: 10000 - 5665 = 4335
            await time.advanceBlockTo('319')
            await this.chef.deposit(0, '10', { from: alice });
            // assert.equal((await this.hbt.totalSupply()).valueOf(), '10000');
            // assert.equal((await this.hbt.balanceOf(alice)).valueOf(), '5099');
            // assert.equal((await this.hbt.balanceOf(bob)).valueOf(), '0');
            // assert.equal((await this.hbt.balanceOf(carol)).valueOf(), '0');
            // assert.equal((await this.hbt.balanceOf(this.chef.address)).valueOf(), '4335');
            assert.equal((await this.chef.getUserRewardInfo(0,alice)).valueOf(),'5099')
            assert.equal((await this.hbt.balanceOf(minter)).valueOf(), '566');
            //(1000*2/7)*0.9 = 257 
            //(1000*2/7)*0.1 = 28 
            await this.chef.extractReward(0, 0, false, { from: bob }); //321
            assert.equal((await this.hbt.balanceOf(bob)).valueOf(), '5356');
            assert.equal((await this.hbt.totalSupply()).valueOf(), '594');
            assert.equal((await this.chef.getUserRewardInfo(0,bob)).valueOf(),'0')
